    ...
    { 
        ...
        public async static Task<TokenResponse> GenerateTokens(string code)
        {
            ...
            try
            {
                ...
                token.AccessToken = dict["access_token"];
                token.RefreshToken = dict["refresh_token"];
                token.ExpiresInSeconds = long.Parse(dict["expires_in"]);
                token.TokenType = dict["token_type"];
